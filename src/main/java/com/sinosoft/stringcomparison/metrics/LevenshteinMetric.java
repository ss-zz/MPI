 ...
 */
public class LevenshteinMetric implements IDistanceMetric {
	...
	public double score(String left, String right) {
		try {
			double ld = (double) ld(left, right);
			return (1 - ld / (double) Math.max(left.length(), right.length()));
		} catch (Exception e) {
			return 0.1;
		}
	}
