...
public class DataUploadController {
	...
	public void dataUploadMonitorDetail(PageInfo page,DataUploadDetail uploadDetail,HttpServletResponse response) throws IOException{
		List<Map<String, String>> list = null;
		try{
	...
	public void dataUploadMonitorBusiness(DataUploadBusiness uploadBusiness ,HttpServletResponse response){
		...
		for (int i = 0; i < tabs.length; i++) {
			sb.append(" (select nvl(sum(upload_count),0) total, cast('" + servids[i]
			            + "' as varchar2(" + servids[i].length() * 3 + ")) servId ");
			...
			if(uploadBusiness.getZonecode() != null && !uploadBusiness.getZonecode().isEmpty() && !uploadBusiness.getZonecode().equals("0000")){//机构
				sb.append(" and c.apanagecode = '"+uploadBusiness.getZonecode()+"'");
			}
			if(uploadBusiness.getOrgcode() != null && !uploadBusiness.getOrgcode().isEmpty() && !uploadBusiness.getOrgcode().equals("0000")){//机构
	...
	public void dataUploadWeekStatistic(String zonecode,String orgcode,HttpServletResponse response){
		StringBuffer sb  = new StringBuffer();
		sb.append(" SELECT A.BS_TYPE BS_TYPE, to_char(A.WEEK,'YYYY-MM-DD') WEEK, NVL(B.TOTALNUM, 0) TOTALNUM			");
