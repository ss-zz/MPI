...
function openViewPage(cfgId){
	...
	var title = '初筛配置详情';
	var url = root+'/cfg/block.ac?method=view&cfgId='+cfgId;
...
function openCurrentPage(){
	...
	var title = '当前初筛配置详情';
	var url = root+'/cfg/block.ac?method=current';
	var name = 'iframe_'+tabId;	
	openTab(tabId,title,url,name);
}
...
function activateCfg(cfgId,cfgName,btn){
	if(confirm("确认使用["+cfgName+"]初筛配置么?\n\r(此配置生效后,现使用的配置将失效)")){
		...
		$.ajax({
			...
			url : root + '/cfg/block.ac?method=effect',// 请求的action路径
...
function openAddPage(){
	var url = root+'/cfg/block.ac?method=toAdd';
